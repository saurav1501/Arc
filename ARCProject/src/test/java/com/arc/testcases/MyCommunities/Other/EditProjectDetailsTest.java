...
public class EditProjectDetailsTest extends BaseClass {
	...
	public void editProjectDetails(int rowNum, String loginSheet, String communitySheet) throws IOException {
		System.out.println(Thread.currentThread().getStackTrace()[1].getMethodName());
		CommonMethod.ExtentReportConfig();
		
		CommonMethod.test = CommonMethod.extent.startTest("Edit Project Detail Test CommOther", "Verifies if Login functionality is working fine").assignCategory("CheckLogin");
		...
		try {
			...
			reuseManage.editProjectDetailsCity(communitySheet, rowNum);

		} catch (Throwable t) {
			System.out.println(t.getLocalizedMessage());
			Error e1 = new Error(t.getMessage());
			e1.setStackTrace(t.getStackTrace());
			CommonMethod.takeScreenshot("editProjectDetailsTest-communitiesOther");
			throw e1;
		}
	}
}
